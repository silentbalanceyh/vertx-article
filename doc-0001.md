	     ...
	     /** 1.提取Token **/
            ...
            if (token.obtained()) {
                ...
                if (null != provider) {
                    ...
                    final Envelop stumer = Envelop.failure(new _500InternalServerErrorException(getClass()));
                }
            } else {
                /** 3.从Header中读取Token失败 **/
...
实际上从上边的几个章节可以知道，如果你了解了Vert.x中的整个结构（如2.4中的图），则可以开发任意自己觉得适合的自定义认证授权框架，但真正在项目过程中，往往使用官方文档的复杂度和我们真正需要的复杂度不成正比，如前文中的复杂代码结构，就应用了很多类似设计的成分。
