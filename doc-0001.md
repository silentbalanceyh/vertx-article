...
*注意：代码示例中的很多代码段来自于真实项目，可能复杂度比简单的DEMO要大，希望读者耐心去品。*
## 术语表
...
这里扩展了原生的username和password两个字段，添加了id（数据库代理主键）和`JsonObject`类型的principal，一般principal存储了一些附加的数据（比如当前用户的角色、基本权限、资源集合等，这个根据实际需要而有所不同）。默认情况下如果要User对象在集群模式中可使用（应该说所有需要在集群模式中使用的数据对象），都会实现`io.vertx.core.shareddata.impl.ClusterSerializable`接口，AbstractUser本身实现了该接口，所以需要针对接口写类似下边的方法：
...
这两个方法只有一个地方要注意，就是**顺序**，上边代码封装了读写Buffer的两个方法（因为代码重复），两个方法的实现如下：
...
从上边代码可以看到，writeToBuffer的顺序是`id, username, password`，那么在readFromBuffer的过程中，其读取的顺序也是`id, username, password`，对于Buffer类型的细节这里不讲，主要是提醒一下大家，如果定义User，必定会重写这两个方法，个人的建议就是把能够标识用户身份的数据写入到Buffer中（如果这些数据需要执行权限检查，那么也需要写入进入）写入的核心点就在于属性字段的顺序，一定要小心（这是序列化和反序列化的操作），顺序出现错误会导致序列化反序列化无法得到你想要的对象。
...
* Authenticator：数据库主体认证访问接口，自定义，用于访问数据库的专用类，而且该类可使用一些设计模式替换其实现，为了和AuthProvider区分开，所以取名为`Authenticator`。
...
2. 私有方法`buildUser`会根据数据库中的返回结果构造对应的BasicUser类；
3. 上边的核心逻辑可以根据读者不同的项目来，比如Record, Authenticator, Options等，这些类的定义不属于Vert.x中的内容；
4. Vert.x中需要实现了仅仅只有方法：authenticate，而这个方法的内部逻辑可以自定义。

### 3.3.令牌类：定义Token

我们在设计权限管理系统时，将Authorization头的信息直接进行了封装，提供了Token接口，该接口的定义如下：
...
而系统中的默认实现为CommonToken，实际上可以使用其他实现，该Token的核心逻辑和上边提到的Vert.x中的parseCredentials类似：
...
定义Token的目的是实现数据对象的传输，让所有的认证专用Provider接收到的信息是一个Token，而不是其他内容，Token本身会转换成它主逻辑中需要使用的JsonObject，个人觉得这样的方式更加适合扩展，目前的实现使用了CommonToken，而了将来可以直接使用其他实现来完成。

### 3.4. 核心Handler类：SecureKeaper

该类实现了前文提到的`Handler<RoutingContext>`，核心代码在`handle(RoutingContext)`中：
...
上述代码是Handler的主逻辑，读者可以只关心代码中的注释部分的流程，不用去关心太多细节。

## 4.总结

实际上从上边的几个章节可以知道，如果你了解了Vert.x中的整个结构（如2.4中的图），则可以开发任意自己觉得适合的自定义认证授权框架，但真正在项目过程中，往往使用官方文档的复杂度和我们真正需要的复杂度不成正比，如前文中的复杂代码结构，就应用了很多设计的成分。不论怎么设计，整理一下请求过程中的流程，希望大家结合2和3章节的例子真正去理解认证和授权的做法。Vert.x中的auth-common是为了简化开发人员的工作，定义了标准的流程，让开发人员可以仅仅通过写AuthProvider和User的实现完成认证（最小改动的做法）。
...
2. 第一步中的解析需要得的几个核心信息：
	...
	2. 当前认证使用的Value信息，不同模式对应的值有所区别，那么解析算法自然会不同。
	3. 一旦不满足上述情况，需要返回401的错误代码。
3. 解析成功过程，需要构造AuthProvider中所需的Credential对象（JsonObject结构），该对象会存储当前认证所需要的一些核心数据，比如Basic模式中包含了username和password的信息，我们在开发过程中针对OAuth2重新开发了一些内容，里边包含了资源ID、交换过后的AccessToken等，由于数据格式是JsonObject，所以这个地方的传输数据非常自由。
4. 【通常在这一步自定义数据库访问，或认证服务器的访问】调用AuthProvider的核心认证方法：authenticate，该方法会将传入的JsonObject转换成User实现对象（该对象可自定义）；
5. 【这一步实现自定义授权验证，包括页面域、数据域、字段域等】在认证通过后，调用User中的doIsPermitted方法执行授权过程，授权过程也可以重写AbstractUser的isAuthorized方法来完整整个流程的设计；
6. 认证成功过后，执行成功逻辑，一般成功的情况会调用RoutingContext的next方法，进入下一个Handler，否则就直接调用fail方法，并且返回对应的状态代码：401-认证、403-授权。

在实现上述流程的过程中，如果不使用Vert.x提供的AuthProvider，你也可以用过程式的代码去实现完整的认证授权逻辑，只要按照上述流程就可以在Vert.x中实现完整的认证授权逻辑，最终只是一个粒度问题。
